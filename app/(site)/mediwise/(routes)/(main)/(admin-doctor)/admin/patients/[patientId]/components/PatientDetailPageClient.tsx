...
const PatientDetailPageClient = () => {
  // const { paientId } = useParams();
  const params = useParams();

  const patientId = params?.patientId as string;
  const patient = useQueryProcessor<TUser & { profile: Profile, barangay: TBarangay }>({
    url: `/users/${patientId}`,
      ...
      <main className="flex flex-col">
        <section className="border-[2px] border-zinc-200 rounded-md p-3 flex items-center">
          <Avatar
            ...
            src={patient?.data?.image as string}
          />
          <div className="flex flex-col ml-3">
            <span className="text-black font-semibold dark:text-white">
              {patient?.data?.profile.firstname} {patient?.data?.profile.lastname}
            </span>
            <span className="text-zinc-500 text-xs">@{patient?.data?.email}</span>
          </div>
        </section>
          ...
          <section className="flex flex-col ">
            ...
            <div className="flex w-full gap-x-3 mt-5">
              <div className="w-full">
                ...
                <div>{patient?.data?.profile.firstname}</div>
              </div>
              ...
              <div className="w-full">
                ...
                <div>{patient?.data?.profile.middlename}</div>
              </div>
            </div>
            ...
            <div className="flex w-full gap-x-3 mt-5">
              <div className="w-full">
                ...
                <div>{patient?.data?.profile.lastname}</div>
              </div>
          ...
          <section className="flex flex-col ">
            ...
            <div className="flex w-full gap-x-3 mt-5">
              <div className="w-full">
                ...
                <div>{patient?.data?.profile.city}</div>
              </div>
              ...
              <div className="w-full">
                ...
                <div>{patient?.data?.profile.homeNo}</div>
              </div>
            </div>
            ...
            <div className="flex w-full gap-x-3 mt-5">
              <div className="w-full">
                ...
                <div>{patient?.data?.profile.street}</div>
              </div>
            ...
            <div className="w-full">
                ...
                <div>{patient?.data?.barangay?.name}</div>
              </div>
              </div>
          </section>
...
export default PatientDetailPageClient;
