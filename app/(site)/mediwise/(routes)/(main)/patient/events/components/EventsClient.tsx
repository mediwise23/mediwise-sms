...
const EventsClient: React.FC<EventsClientProps> = ({currentUser}) => {
  ...
  const events = useQueryProcessor<(EventSchemaType & {barangay: TBarangay})[]>({
    url: `/events`,
    ...
    <div className="w-full ">
      ...
      <section className="bg-[#212121] w-full px-40 overflow-auto h-screen ">
        <div className="gap-y-10 flex flex-col">
          <h1 className="capitalize text-white text-3xl mt-10">Upcoming events</h1>
          <Separator />
        </div>
        ...
        {/* events */}
        {
          (() => {
            if (events.status === 'pending') {
              return <div className="flex justify-center"><Loader2 size={50} /></div> 
            }
            ...
            return <section className="flex flex-col py-10 gap-y-5">
